 ...
 *
 * <h5>Note:</h5>
 * <p>XmlBeanFactory使用了自定义的XML读取器XmlBeanDefinitionReader，实现了个性化的BeanDefinitionReader读取。
 * <p>XmlBeanFactory对DefaultListableBeanFactory类进行了扩展，主要用于从XML文档中读取BeanDefinition，
 * 对于注册及获取都是从父类DefaultListableBeanFactory继承的方法去实现，而唯独与父类不同的个性化实现就是增加XmlBeanDefinitionReader类型的reader属性。
...
public class XmlBeanFactory extends DefaultListableBeanFactory {
	 ...
	 * which must be parsable using DOM.
	 * @param resource XML resource to load bean definitions from
	 * @param parentBeanFactory parent bean factory 为父类BeanFactory用于factory合并，可以为空
	 * @throws BeansException in case of loading or parsing errors
	 */
