...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	/**
	 * 默认标签的解析：分别对4种不同标签（import、alias、bean和beans）做了不同的处理。
	 * @param ele
	 * @param delegate
	 */
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if (absoluteLocation) { // 如果是绝对URI，则直接根据地址加载对应的配置文件
			/**
			...
			/**
			 * 5.如果是相对路径，则根据相对地址计算出绝对路径，并解析。
			 */
			try {
				int importCount;
				// Resource存在多个子类实现，如VfsResource、FileSystemResource等，
				...
				if (relativeResource.exists()) {
					...
					// 如果解析不成功，则使用默认的解析器ResourcePatternResolver
		...
		getReaderContext().fireImportProcessed(location, actResArray, extractSource(ele)); // 解析后，进行监听器激活处理
	}
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		if (bdHolder != null) {
			...
			// 当需要对注册BeanDefinition事件进行监听时可以通过注册监听器的方式并将处理逻辑写入监听器中，目前Spring中并没有对此事件做任何逻辑处理。
